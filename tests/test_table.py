...
def test_basic_1_field_list(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/FlexLiftExport.lift")
   t = TableSet(lift, ["ID", "form", "morphtype", "gloss"])
   assert len(t.fields_by_level[LiftLevel.ENTRY]) == 3
   assert len(t.fields_by_level[LiftLevel.SENSE]) == 1

def test_short(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/FlexLiftExport.lift")
   t = TableSet(lift, ["ID", "form", "morphtype"])
   table = t.get_level_table(LiftLevel.ENTRY)
   assert table.shape[0] == lift.get_n(LiftLevel.ENTRY)

def test_convert_level_table_to_csv(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/FlexLiftExport.lift")
   t = TableSet(lift, ["ID", "form", "morphtype"])
   table = t.get_level_table(LiftLevel.ENTRY)
   table.to_csv("test.csv")

def test_convert_csv(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/FlexLiftExport.lift")
   t = TableSet(lift, ["ID", "form", "morphtype"])
   a = AggregatedTable(t)
   a.to_csv(tmp_path / "test_table_to_csv.csv")

def test_convert_cldf(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/FlexLiftExport.lift")
   t = TableSet(lift, ["ID", "form", "morphtype"])
   a = AggregatedTable(t)
   a.to_cldf_Wordlist("cldf")

def test_get_entry_table(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/FlexLiftExport.lift")
   t = TableSet(lift, ["ID", "form", "morphtype","gloss"])
...
def test_merge_aggregate(capsys, tmp_path):
...
def test_merge(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/tiny.lift")
   t = TableSet(lift, ["ID", "form", "morphtype", "gloss"])
   a = AggregatedTable(t)
   table = a.get_table()
...
def test_merge_large(capsys, tmp_path):
   lift:LiftDoc = LiftDoc("tests/data/FlexLiftExport.lift")
   t = TableSet(lift, ["ID", "form", "morphtype", "gloss"])
   a = AggregatedTable(t)
